...
for da_run, icon_emvorado_run, spin_up_mm, short_name in zip(
    ...
    hm='h'
    qvp_3_syn = syn_nc['vol_q' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time')*1000
    qvp_3_syn=qvp_3_syn.where(syn_nc['D0_' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time')>0,np.nan)
    qvp_3_syn=qvp_3_syn.where(syn_nc['vol_qn' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time')>0,np.nan)

    hm='i'
    qvp_4_syn = np.log10(syn_nc['vol_qn' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time')/1000)
    qvp_4_syn=qvp_4_syn.where(syn_nc['D0_' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time')>0,np.nan)
    qvp_4_syn=qvp_4_syn.where(syn_nc['vol_q' + hm].sel(
        time=slice(date_start, date_end)) \
        .transpose(..., 'time'),np.nan)
    ...
    plot_qvp_of_polarimetric_variable(
        ...
        xlabel=xlabel,
        ylabel='',
        panel= letters[letters_i] +') ' + short_name + ' (hail)',
    )
    letters_i=letters_i+1
    # ------------------------------------ #
    current_col = current_col +1
