...
icon_emvorado_runs.append('MAIN_2411.0/EMVO_20010000.2off')
spin_up_mms.append('120')
short_names.append('I1E1')
colors.append(mpl.colormaps._cmaps['HomeyerRainbow'](np.linspace(0, 1, 5))[0])
colors2.append('blue')
# ------------------------------------ #
# # SYN data row 2                       #
# # ------------------------------------ #
...
# SYN data row 5                       #
# ------------------------------------ #
# for f in QVPqnx*-116.5e-05.nc; do
#     base="${f%-116.5e-05.nc}"
#     start="${base%qnx*}"
#     rest="${base#*qnx}"
#     new="${start}qnxoff${rest}.nc"
#     echo ${base}
#     echo ${start}
#     echo ${rest}
#     mv "$f" "$new";
    ...
    if icon_emvorado_run[-3:] == 'qnx':
        ...
        path_mod = '/'.join(
            [header.dir_data_qvp + date, da_run, icon_emvorado_run[:-6],
             ...
             str(elevation_deg) + '_Syn_' + location + '_' +
             date + '0000_' + date + '2355.nc'])
    elif icon_emvorado_run[-3:] == 'off':
        path_mod = '/'.join(
            [header.dir_data_qvp + date, da_run, icon_emvorado_run[:-3],
             str(spin_up_mm) + 'min_spinup', 'QVPoff_' +
             str(elevation_deg) + '_Syn_' + location + '_' +
             date + '0000_' + date + '2355.nc'])
    else:
        path_mod = '/'.join(
...
plt.savefig(
    ...
    '3.png', format='png', transparent=False, dpi=300, bbox_inches='tight')
plt.savefig(
    folder_plot +
    '/QVPs_Fig_9_syn_ret_' + str(n_rows) + 'x3rainretrievals_QVP_' +
    str(elevation_deg) + '°_' +
    ...
    ['', str(filter_entr_at) + '_'][filter_entr] +
    ['', 'mom_'][filter_moms] + mod_names[1:] +
    '3.pdf', format='pdf', transparent=True)
plt.close()
...
# ------------------------------------ #
bins_dr=40  # 50 # default
mod_names = ''
letters_i=0
n_rows = 1*len(da_runs) + 1 + 1 +1 # add one once more for mean of all
n_cols = 3
factor=0.7
...
plt.savefig(
    ...
    ['', 'mom_'][filter_moms] + mod_names[1:] +
    ['', '_2e14'][testing] +
    '3.pdf', format='pdf', transparent=True, bbox_inches='tight')
plt.savefig(
    folder_plot +
    ...
    ['', 'mom_'][filter_moms] + mod_names[1:] +
    ['', '_2e14'][testing] +
    '3.png', format='png', transparent=True, dpi=300, bbox_inches='tight')
plt.close()
