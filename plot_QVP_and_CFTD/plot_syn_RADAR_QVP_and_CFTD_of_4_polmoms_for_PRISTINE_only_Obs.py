...
# ------------------------------------ #
# QVPs row 1 parameters                #
# ------------------------------------ #
location = 'ESS'
date = '20181223'
hhmm_start_qvp = '03:59'
hhmm_end_qvp = '23:59'
min_azimuth=False
max_azimuth=False
if max_azimuth:
    name_extension = '(' + str(min_azimuth) + ',' + str(max_azimuth) + ')5'
else:
    name_extension = '5'
...
if len(path_obs) == 1:
    ...
    path_obs = glob.glob(path_obs + '_'.join(
        ['/ras*qvp'+name_extension+'*', '*_polmoms_nc_*' + date + '*' + location.lower() + '*']))
    if len(path_obs) == 1:
        path_obs = path_obs[0]
        print(path_obs)
    else:
...
plot_qvp_of_polarimetric_variable(
    ...
    panel=letters[letters_i] + ') ' + location + ' ' + name_extension[:-1],
)
...
if len(path_obs) == 1:
    path_obs = path_obs[0]
    path_obs = glob.glob(path_obs + '_'.join(
        ['/ras*qvp' + name_extension + '*',
         '*_polmoms_nc_*' + date + '*' + location.lower() + '*']))
    if len(path_obs) == 1:
        path_obs = path_obs[0]
        print(path_obs)
    else:
...
hh_at=np.arange(int(hhmm_start_qvp[:2])+3, int(hhmm_end_qvp[:2]), 2)
...
plt.savefig(
    folder_plot +
    '/QVPs_PRISTINE' + str(n_rows) + 'x4polmoms_' +
    str(elevation_deg) + '°_' +
...
# filter_entr_at = 0
# filter_moms = False
# folder_plot = header.folder_plot + 'PPRISTINE/'
#
# # ------------------------------------ #
# # MODELS                               #
# # ------------------------------------ #
...
# short_names.append('2405.3-00510000.2')
# colors.append('red')
# # # ------------------------------------ #
# # # SYN data row 2                       #
# # # ------------------------------------ #
# da_runs.append('ASS_2407')
# icon_emvorado_runs.append('MAIN_2405.3/EMVO_00512000.2')
# spin_up_mms.append('120')
# short_names.append('2405.3-00512000.2')
...
# spin_up_mms.append('120')
# short_names.append('2405.3-00513000.2')
# colors.append('green')
#
# # --------------------------------------------------------------------------- #
#
