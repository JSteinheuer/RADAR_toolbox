...
qvp_1_obs = obs_nc['Dm_totice'].sel(
    time=slice(date_start, date_end)).transpose(..., 'time')
qvp_2_obs = obs_nc['IWC'].sel(
    time=slice(date_start, date_end)).transpose(..., 'time')
qvp_3_obs = obs_nc['Nt_totice'].sel(
    time=slice(date_start, date_end)).transpose(..., 'time')
...
plt.savefig(
    folder_plot +
    '/QVPs_' + str(n_rows) + 'x3iceretrievals_PPI_' +
    str(elevation_deg) + '°_' +
...
print(current_col)
ax = axs[current_row, current_col]
x ,y = plot_CFAD_or_CFTD_from_PPI_with_list(
    locations=locations,
    dates=dates,
...
plt.savefig(
    folder_plot +
    '/CFTDs_' + str(n_rows)+ 'x3iceretrievals_PPI_' +
