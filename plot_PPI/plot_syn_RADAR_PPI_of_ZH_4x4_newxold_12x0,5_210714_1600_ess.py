...
file_out = ('2x2_PPI_zh_12x05_new_old_' + str_location + '_' + str_mod +
            '.' + pdf_or_png)
plt.tight_layout()
...
path = '/'.join([header.dir_data_vol[:-1], date, da_run,
                 ...
                 date + hhmm_start + '_' + date + hhmm_end + '.nc'])
syn_nc = xr.open_dataset(path)
syn_nc=syn_nc.sel(elevation=elevation_deg)
syn_nc=syn_nc.isel(time=[time_i_mod,time_i_mod+1])
# ------------------------------------- #
# ------------------------------------- #
da_run = 'ASS_2411'
icon_emvorado_run = 'MAIN_2411.3/EMVO_20510000.2'
...
short_name =  (short_name + ' - ' +
               icon_emvorado_run[-17:-15] + icon_emvorado_run[-10:])
year = date[0:4]
mon = date[4:6]
day = date[6:8]
hhmm_start = str(time_hhmm - time_hhmm % 600).zfill(4)
...
path = '/'.join([header.dir_data_vol[:-1], date, da_run,
                 ...
                 'min_spinup/EMV_Vol_' + location.upper() + '_' +
                 date + hhmm_start + '_' + date + hhmm_end + '.nc'])
syn_nc2 = xr.open_dataset(path)
syn_nc2=syn_nc2.sel(elevation=elevation_deg)
syn_nc2=syn_nc2.isel(time=[time_i_mod,time_i_mod+1])
# ------------------------------------- #
time_i_mod=0
# ------------------------------------- #
moment_i=0
...
levelss = [header.levels_zh, header.levels_zdr,
           header.levels_kdp, header.levels_rhohv,]
moment = moments[moment_i]
nc_file_comb[moment]=syn_nc[moment]-syn_nc2[moment]
cmap = cmaps[moment_i]
norm = norms[moment_i]
levels =levelss[moment_i]
n_i=(row-1)*n_cols+col
...
letters_i = col + (row-1)*(n_cols)
if mode == 'vol':
    title = (letters[letters_i] +') Diff ' + short_name +
             ': ' + moment[:-3] + ' ' +
             str(elevation_deg) + '° ' +
             location.upper() + ' ' + time_UTC_mod[6:-3])
else:
    title = (letters[letters_i] +') Diff ' + short_name +
             ': ' + moment[:-3] + ' pcp ' +
             location.upper() + ' ' + time_UTC_mod[6:-3])
...
str_mod = '_'.join(path.split('/')[-5:-2]) + '_' + str(spin_up_mm)
str_location = '_'.join([location.upper(), date, str(time_hhmm)])
file_out = ('2x1_PPI_Diff_zh_12x05_new_old_' + str_location + '_' + str_mod +
            '.' + pdf_or_png)
plt.tight_layout()
if not os.path.exists(folder_plot):
    os.makedirs(folder_plot)
