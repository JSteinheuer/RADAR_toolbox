...
n_cols = 5
# factor = .55
# fig = plt.figure(figsize=(factor *n_cols * 3.5, factor *n_rows * 4.25), layout='constrained')
factor = .52
fig = plt.figure(figsize=(factor *n_cols * 3.5, factor *n_rows * 4.25), layout='constrained')
...
for moment, cmap, norm, levels, label in zip(
    ...
    cbar=fig.colorbar(pcm,  location='top', label=label, fraction=.05,
                      ticks=levels)
...
file_out = 'Fig_8_' + str(elevation_deg) + \
