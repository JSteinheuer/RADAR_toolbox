...
# path = folder_obs + 'ras07-qvp5minng01_sweeph5onem_z_polmoms_das2_' \
#                     'nc_07-202107140003-202107142358-ess-10410.hd5'
# obs_nc = dttree.open_datatree(path)[
#         'sweep_' + str(int(7))].to_dataset().chunk(-1)
path = folder_obs + 'ras07-qvp5minng01_sweeph5onem_polmoms_' \
                    'nc_07-202107140003-202107142358-ess-10410.hd5'
obs_nc = xr.open_dataset(path)
top_height = 10
h_obs = obs_nc['height']
...
plot_qvp_of_polarimetric_variable(
    ...
    levels_cf=np.arange(-50, 60, 5),
    cbar_title=r'Differential Reflectivity [dB]',
    # title='Z$_{DR}$ DAS2 QVP (C-band Obs. at ' + location + ')',
    title='Z$_{DR}$ comb 2 QVP (C-band Obs. at ' + location + ')',
    ax=plt.subplot(n_rows, n_cols, n_i),
    mom_height_unit='km',
    scale_font=2.6,
    scale_numbers=2,
...
plot_qvp_of_polarimetric_variable(
    mom=qvp_zdr_obs,
    cmap=header.cmap_radar22,
    norm=header.norm_zdr22,
    levels=header.levels_zdr,
    mom_cs=None,  # qvp_zh_obs,
    levels_cs=np.arange(-50, 60, 5),
    mom_cf=qvp_temp_obs,
...
plot_qvp_of_polarimetric_variable(
    ...
    cbar_title=r'Differential Reflectivity [dB]',
    # title='Z$_{DR}$ DAS2 QVP (C-band Obs. at ' + location + ')',
    title='Z$_{DR}$ comb2 QVP (C-band Obs. at ' + location + ')',
    ax=plt.subplot(n_rows, n_cols, n_i),
    mom_height_unit='km',
    scale_font=2.6,
    scale_numbers=2,
...
plt.savefig(
    folder_plot + 'QVP_zdr_b' + str(n_cols) + '_' + str(elevation_deg) + '_' +
