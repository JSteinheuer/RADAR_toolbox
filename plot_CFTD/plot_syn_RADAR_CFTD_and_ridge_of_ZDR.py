...
gg = sns.FacetGrid(df, row="temp", hue="temp", aspect=15,
               height=.4, palette=pal,ylim=[0,.6], xlim=[-1,6.1]) # zdr obs
...
# gg.set(xlabel='$Z_{DR}$ [dB]')
# ax = plt.gca()
ax.text(.5, 14, letters[letters_i] + ') obs',  transform=ax.transAxes)
...
for da_run, icon_emvorado_run, spin_up_mm, color, short_name in zip(
    ...
    gg.set(xlabel='$Z_{DR}$ [dB]')
    ax = plt.gca()
    ax.text(.5, 14, letters[letters_i] + ') '+ short_name, transform=ax.transAxes)
    letters_i = letters_i + 1
