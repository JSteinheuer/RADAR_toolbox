...
def sector_qvp_from_radar_PPIs(date='20170725', elevation_deg=12,
                               location='pro',
                               dir_data_obs=header.dir_data_obs,
                               dir_data_out=header.dir_obs_qvp,
                               ml_correction=False,
    ...
    name_extension='(' + str(min_azimuth) + ',' + str(max_azimuth) + ')'
    # load
    data = dttree.open_datatree(path_in)[
        'sweep_' + str(int(sweep))].to_dataset().chunk(-1)
    data = data.transpose('time', 'azimuth', 'range')
    ...
    if min_azimuth>max_azimuth:
        data=data.where(((data.azimuth >= min_azimuth) |
                        (data.azimuth <= max_azimuth)), drop=True)
    else:
        data=data.where(((data.azimuth <= max_azimuth) &
                        (data.azimuth >= min_azimuth)), drop=True)
    ...
    filter_comments = (filter_comments + 'QVP needs at least ' + \
                      str(n_lowest) + ' valid valuesout of the ' +
                       name_extension+ ' azimuth window per median ' \
                                      'calculation on azimuth. ')
    qvp_first_nc = qvp_first_nc.swap_dims({'range': 'height'})
    # ------------------------------------------------------------------- #
    # QVP: Melting layer correction: TS method; not really working        #
