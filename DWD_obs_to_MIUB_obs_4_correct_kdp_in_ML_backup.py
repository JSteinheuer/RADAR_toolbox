...
def proc_phidp_kdp(swp_cf, uh_tresh=0, rho_tresh=0.8, snr_tresh=10,
    ...
    # 1: centering PHI first guess (around modus (of lower range bins)):
    # phi_hist = np.histogram(swp_mask.UPHIDP.copy().where(
    #     swp_mask.range < 10000).values.flatten(),  # TODO: 10000?
    phi_hist = np.histogram(swp_mask.UPHIDP.copy(),
                            # phi_hist = np.histogram(UPHIDP_factor.copy(),
                            bins=np.linspace(-180, 180, 361))
    phi_modus = np.argmax(phi_hist[0]) - 180
    print(phi_modus)  # TODO
    phi_c = ((swp_mask.UPHIDP.copy() - xr.DataArray(
        phi_modus) - 100)  # TODO: very new
             % 360 + 180) % 360 - 180

    # 2: smoothing PHI
    phi_s = phi_c.copy().pipe(
        xr_rolling, window=win_r, window2=win_azi, method="median",
        skipna=True, min_periods=max(3, int((win_r - 1) / 6))  # TODO: 3 o 1/6?
    )

    # 3: offset Part 1 of 7: calculate offset 2d
    phi_off_2d = phase_offset(phi_s.copy().where(swp_cf.range > 1000),
                              # TODO works
                              # TODO: 1000?
                              rng).PHIDP_OFFSET.load()

    # 3: offset Part 2 of 7: filter outliers (>3 in neighbourhood)
    phi_off_2d_f = xr.where(np.isnan(phi_off_2d), -100,
                            phi_off_2d)  # TODO: very new -100 instead of 0
    phi_off_2d_f = xr.DataArray(uniform_filter(phi_off_2d_f,
                                               size=5, mode='mirror'),
                                dims=['time', 'azimuth'])
    phi_off_2d_f = phi_off_2d.where(
        (abs(phi_off_2d - phi_off_2d_f)) < 3)  # TODO: 3?

    # 3: offset Part 3 of 7:  interpolate na's
    phi_off_2d_f_i = phi_off_2d_f.interpolate_na(dim='time', limit=5)

    # 3: offset Part 4 of 7:  median offsets per time
    ...
    # 3: offset Part 5 of 7:  filling gaps with 0 (after centering)
    # 3: offset Part 5 of 7:  filling gaps with -100 (after centering)
    phi_off_2d_s = phi_off_2d_f_i - phi_off_1d
    phi_off_2d_s = xr.where(np.isnan(phi_off_2d_s), 0,
                            phi_off_2d_s)  # TODO: very new -100 instead of 0
    phi_off_2d_s = phi_off_2d_s + phi_off_1d

    # 3: offset Part 6 of 7:  smoothing
    phi_off_2d_c_s = xr.DataArray(
        gaussian_filter(phi_off_2d_s, [.5, 3],  # TODO: time=.5, azimuth=3?
                        mode=['mirror', 'wrap', ]),
        dims=['time', 'azimuth'])

    # 3: offset Part 7 of 7:  noise corrected phi
    phi_nc = phi_s - phi_off_2d_c_s
    ...
    swp_cf = swp_cf.assign(phi_off_2d_s=phi_off_2d_c_s + phi_modus + 100)
    ...
    swp_cf = swp_cf.assign(phi_off_2d_c_s=phi_off_2d_c_s + 100)
