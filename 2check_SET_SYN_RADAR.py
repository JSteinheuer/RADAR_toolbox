...
def get_lon_lat_alt(r, az, el, sitecoords):
    ...
    cent_coords = wrl.georef.spherical_to_centroids(r, az, el, sitecoords,
                                                    crs=proj_wgs84)  # TODO: check: curvature included
...
def ipol_fc_to_radgrid(mod_lon, mod_lat, mod_z, rad_lon, rad_lat, rad_alt,
    ...
    src = np.vstack((mod_x.ravel(),
                     mod_y.ravel(),
                     mod_alt.ravel() / 1e3)).T  # km should be appropriate
    trg = np.vstack((rad_x.ravel(),
                     rad_y.ravel(),
                     rad_alt.ravel() / 1e3)).T  # km should be appropriate
    if method == 'Linear':
...
# create_8_vol_nc_of_day(day=day,
#                        da_run=da_run,
#                        icon_run=icon_run,
...
# dir_data_out='/kais/folder/insert/name/here/'

radar_loc='PRO'
method='Nearest'
time_start = pd.to_datetime(day, format="%Y%m%d")
# for i in range(4):
i = 0 # only one time step
time_end = time_start + pd.Timedelta('5min') # '6h')
...
if ('vol_scan' not in locals() and include_icon
    ...
    rad_alt = np.repeat(vol_scan['alt'].data[:, np.newaxis, :],  # TODO check change
